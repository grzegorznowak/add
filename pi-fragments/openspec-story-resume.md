...
Retrieve `notebook_read({name: "openspec-research-<initiative_slug>-<story_slug>"})` when a converger names that page. If an entry does not verify, report a `## Research Events` notebook-refresh signal with correction anchors in the final response; do not curate converger-provided notebook entries directly.

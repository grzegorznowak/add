...
`spawn({prompt: "Search the codebase for <keywords> for story <epic>/<step>. Report paths, roles, reusable code, gotchas. Write findings to notebook page 'plan-probe-<epic>-<step>' with path:line anchors.", thinking: "medium"})`.

Retrieve findings: `notebook_read({name: "plan-probe-<epic>-<step>"})`. Integrate into spec sections.

### Handoff for multi-session planning
When planning spans multiple sessions (e.g., Mode A → Mode B), use `handoff()` between modes. Brief should contain: modes completed, sections edited, remaining incomplete sections, notebook page names.

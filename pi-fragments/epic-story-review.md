...
`spawn({prompt: "Focused review pass: <title> for <epic>/<step>. Inspect <files/symbols> for acceptance items <A<n>>. Be read-only. Write findings to notebook page 'review-pass-<epic>-<step>-<slugged-title>' using the detailed finding card format, including code surfaces searched and evidence quality. Return summary.", thinking: "high"})`.

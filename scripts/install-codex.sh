...
preflight_content_if_safe() {
  ...
  tmp="$(mktemp "${TMPDIR:-/tmp}/codex-preflight.XXXXXX")"
  printf '%s\n' "$content" > "$tmp"
...
codex_args() {
  ...
  case "$skill" in
    ...
    openspec-initiative-plan)
      ...
      ;;
    openspec-migrate)
      arg_line='s/^Argument:.*/Argument: INITIATIVE=<slug> [STORY=<slug>]/; s/^argument-hint:.*/argument-hint: "INITIATIVE=<slug> [STORY=<slug>]"/'
      body_repl='s/\$ARGUMENTS/the INITIATIVE and STORY named variables/g'
      ;;
...
for skill_dir in "$CLAUDE_SKILLS"/*/; do
  ...
  if [[ "$skill_name" == "openspec-migrate" ]]; then
    # Treat the generated skill, its Codex metadata, and helper as one unit:
    # detect every overwrite conflict before creating or replacing any file.
    preflight_content_if_safe "$out_dir/SKILL.md" "$stripped"
    preflight_content_if_safe "$out_dir/agents/openai.yaml" "$metadata"
  ...
  write_content_if_safe "$out_dir/agents/openai.yaml" "$metadata"
  if [[ "$skill_name" == "openspec-migrate" ]]; then

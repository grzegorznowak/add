...
- Inspect the relevant code and tests before the first change. Use the story's
  `## Actors`, `## Scenarios / Behavior Examples`, `## Verification`,
  `## Critical Files`, and `## Discovery Notes` to choose the smallest focused
  seam that covers the next behavior.
- Before choosing the first red seam, build a compact acceptance proof ledger
  ...
  acceptance id. Do not treat one variant or scenario test as covering siblings
  unless the story explicitly excludes them.

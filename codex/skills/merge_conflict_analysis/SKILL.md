...
description: Analyze whether one git ref merges cleanly into another and trace conflicts to PR, Jira, or ticket-backed intent before recommending a resolution.
---

Merge Conflict Analysis: $REPO / $FROM / $INTO / $TEMP_ROOT

Treat `$REPO` as the target git repository, `$FROM` as the incoming branch or ref to merge, and `$INTO` as the branch or ref that would receive the merge. Treat `$TEMP_ROOT` as optional; default it to `/tmp` when omitted. Any throwaway checkout or worktree must live below `$TEMP_ROOT`.

Your job is to determine whether merging `$FROM` into `$INTO` is clean. If it conflicts, explain the real conflicts by tracing commits to PRs, Jira issues, business stories, or another reliable change-intent source. This is analysis-only: do not implement the resolution, commit, push, or leave the repository changed.
...
1. Parse the invocation strictly as:
   - `$REPO`: target git repository.
   - `$FROM`: incoming branch or ref to merge.
   - `$INTO`: branch or ref that would receive the merge.
   - `$TEMP_ROOT`: optional temporary root, default `/tmp`.

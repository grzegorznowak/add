---
name: epic-pr
description: Open or refresh a GitHub PR for an entire epic, using CONTRACT.md and current DONE stories to produce an epic-level reviewer body. Use when completed epic scope should be sent to GitHub review as one coherent PR.
disable-model-invocation: true
argument-hint: "<epic-name-or-path> [pr-url|OPEN=true]"
allowed-tools: Read Edit Write Grep Glob Bash(git status:*) Bash(git branch:*) Bash(gh pr view:*) Bash(gh pr list:*) Bash(gh pr edit:*) Bash(gh pr create:*) Bash(curl:*)
---

# Epic PR

Open or refresh a GitHub PR for an entire epic. This is an epic-level check-in and PR-body workflow: it gathers the already-squashed contract plus current non-archived DONE stories, detects blocking coordination-doc gaps or conflicts, optionally applies operator-directed doc fixes, then publishes a product-focused PR body only when the epic is coherent enough to review.

Argument: `$ARGUMENTS` — `<epic-name-or-path> [<pr_url_or_OPEN=true>]`. The epic may be a directory name under `<cwd>/agent_coordination/epics/` or a direct path to an epic directory. The second argument is either a full GitHub PR URL or the literal `OPEN=true`; if omitted, infer an existing PR first, then ask before opening a new one.

## Phase 0 — Resolve epic and PR intent

1. Resolve the epic directory:
   - If the first argument is a path containing `MASTER.md`, use it.
...
3. Resolve the PR operation:
   - If a PR URL was passed, use attach mode.
   ...
   - If none is found, ask before opening a new PR via `gh pr create`. The operator does not need to type `OPEN=true`.
...
Never read archived story files directly. If `MASTER.md` has archived DONE story links and `CONTRACT.md` is missing, block early before drafting: archived scope has no allowed contract source.

Abort if there is no included source at all: no `CONTRACT.md` and no non-archived `✅ DONE` stories.

Read only product-facing sections from non-archived DONE stories:

- `## Purpose`
- `## Actors` when role context is product-facing
- normative `## Scenarios / Behavior Examples` linked with `Covers: A<n>`
- `## Triggering Need`
- `## Scope`
- `## Out of Scope`
- `## Acceptance`
...
Do not read or use `## Verification`, `## Active Claim`, `## Progress Log`, `## Session Handoff`, `## Review Log`, or implementation diary sections. A `✅ DONE` story is trusted as already reviewed.

## Phase 2 — Original ticket link pass

Find explicit original ticket/card links or stable ticket identifiers in `MASTER.md`, `CONTRACT.md`, and included non-archived DONE story prose.
...
Draft a product-focused PR body from the included sources. Merge story inputs into one epic-level narrative; do not create story-by-story sections.
...
Omit `## Original tickets` if there are no links. Do not include verification commands, proof matrices, unfinished story details, or implementation diary content. Mention unfinished stories only in the check-in and final response, not in the PR body.

## Phase 4 — Secondary epic check-in

Before creating or editing the PR, run a lightweight coordination-doc consistency check. This is not codebase verification and not `/epic-squash`.
...
If blockers exist, do not create or edit the PR and do not update `## Epic PR Tracking`. Enter Phase 5.

If no blockers exist, ask the operator to publish the drafted body. If the operator declines, abort without PR or `MASTER.md` write-back.

## Phase 5 — Operator-led repair loop

When blockers exist, let the operator decide what happens next. Do not recommend `/epic-squash` as the default fix and do not invoke other skills.
...
If `gh` fails, abort without `MASTER.md` write-back unless a PR URL is already known and metadata was refreshed successfully.
...
4. **Never transition story statuses.** `/epic-story-pr` owns story PR state.

...
- `## Verification` — exists, has both `### Verification Commands` and `### Acceptance Proof Matrix` subsections; the matrix covers every `A<n>` id and every named variant/failure mode inside an id; activated risk lenses are covered by existing matrices or a `### Risk Lens Inventory` with explicit exclusions; when the story references design sources, `### Design Sources` is present, and every normative source has a complete `### Design Element Trace`.

...
(Skip this question if `NAME` was passed as an argument — use it as the slug and confirm the human title in the drafting step.)
...
(Informal list of planned stories — these become `/epic-story-plan` invocations later, one per story. The tracker table below stays empty until a story is actually drafted.)
...
/epic-plan does not overwrite existing epics. If you meant to:
  - add stories to this epic → /epic-story-plan EPIC=<slug>
  - restart from scratch     → rm -rf agent_coordination/epics/<slug>/
  - use a different slug     → re-run /epic-plan NAME="<different-slug>"
```

...
- Implementation review completion is represented by the top-level `Status:` header plus the single current `progress.md → ## Implementation Review Receipt` record. Review replaces that section rather than appending receipt history. Only an unbound pre-v3 DONE with zero Initiative headers and zero receipt sections is legacy; never create a retroactive receipt here. Optional notebooks are implementation orientation only and never review/lifecycle authority.
...
10. Inventory every `<progress_file> → ## Implementation Review Receipt` when present. One well-formed unsuperseded section is current; traceably superseded material is historical. Duplicate/malformed sections are not current authority and must be normalized by a fresh substantive `/openspec-story-review` when the story is reviewable; this converger never rewrites them or launches review. Keep the exact `WORKTREE=` fragments for later command lines. Do not normalize or reinterpret them except for the read-only OpenSpec-root preflight above.
...
`progress.md` normally contains exactly one `## Implementation Review Receipt`, the current review record; review replaces all receipt sections with one normalized section rather than appending history. Optional notebooks never replace it.
...
- `Status: 🟣 IN REVIEW` is review-ready when current implementation/proof evidence or an authorized reopen/resume/unblock event is newer than a prior non-approve receipt, no named repair remains, and review has not run against that current evidence. Duplicate/malformed receipt sections do not permanently reject review: stop with the singular fresh substantive `/openspec-story-review` route, which owns normalization to exactly one current receipt.
- `Status: ✅ DONE` on a bound story requires exactly one well-formed current receipt with every required field exactly once, `Decision: APPROVE`, `Approval gate: PASS`, and a transition ending in `✅ DONE`. Do not recompute review identity here: it is story-scoped delivery evidence for PR, not lifecycle or prerequisite state. A missing, duplicate, malformed, or non-approving bound-story receipt routes singularly to fresh substantive `/openspec-story-review` for normalization. Only an unbound pre-v3 DONE story with zero Initiative headers and zero receipt sections is bounded legacy fallback; warn, spot-check existing task/proof evidence, and never synthesize/request a receipt. No other status may infer completed review from a notebook.

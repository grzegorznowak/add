...
- Implementation review completion is represented by the top-level `Status:` header plus the single current `progress.md → ## Implementation Review Receipt` record. Feedback replaces that section from a validated completed-review handoff rather than appending receipt history. Only an unbound pre-v3 DONE with zero Initiative headers and zero receipt sections is legacy; never create a retroactive receipt here. Optional notebooks are implementation orientation only and never review/lifecycle authority.
...
10. Inventory every `<progress_file> → ## Implementation Review Receipt` when present. One well-formed unsuperseded section is current; traceably superseded material is historical. Duplicate/malformed sections are not current authority. For bound modern DONE they use the ordinary-feedback reopen route below; after repair, fresh substantive review authors a new handoff and feedback normalizes during publication. This converger never rewrites them or launches review. Keep the exact `WORKTREE=` fragments for later command lines. Do not normalize or reinterpret them except for the read-only OpenSpec-root preflight above.
...
- `Status: 🟣 IN REVIEW` is a successful implementation-convergence stop only when bounded readiness evidence shows no named repair issue and review has not yet run against the current evidence. An aborted review's preserved status does not override its named implementation/proof, contract, or external-evidence repair owner. A prior non-approve receipt may be historical after a newer authorized feedback reopen, resume/proof update, or unblock transition and must not deadlock current IN REVIEW routing.
...
### Implementation review receipt routing

`progress.md` normally contains exactly one `## Implementation Review Receipt`, the current review record; feedback replaces all receipt sections from a validated completed-review handoff with one normalized section rather than appending history. Optional notebooks never replace it.

- For every non-DONE state, authoritative Status plus the `blocked.md` gate owns routing first. A prior receipt is historical when a newer authorized feedback reopen, resume claim/timeline or proof update, or unblock transition traces the supersession. Use its findings as context, but do not let historical, duplicate, or malformed receipt material deadlock `🔄 IN PROGRESS` when that supersession is traceable.
- Without traceable supersession, `Decision: REQUEST CHANGES` normally pairs with `Status: 🔄 IN PROGRESS`, and `Decision: BLOCKED` with `Status: ⛔ BLOCKED` plus `<blocked_file>`. An unexplained conflict requires reconciliation; never choose whichever source is convenient.
- `Status: 🟣 IN REVIEW` is review-ready when current implementation/proof evidence or an authorized reopen/resume/unblock event is newer than a prior non-approve receipt, no named repair remains, and review has not run against that current evidence. Duplicate/malformed historical receipt sections do not permanently reject review: stop with the singular fresh substantive `/openspec-story-review` route. Review authors the normalized completed-review handoff; `/openspec-feedback` validates it and publishes exactly one current receipt.
- `Status: ✅ DONE` on a bound story requires exactly one well-formed current receipt with every required field exactly once, `Decision: APPROVE`, `Approval gate: PASS`, and a transition ending in `✅ DONE`. Do not recompute review identity here: it is story-scoped delivery evidence for PR, not lifecycle or prerequisite state. Invalid receipt evidence uses the ordinary-feedback reopen route stated in this skill. Only an unbound pre-v3 DONE story with zero Initiative headers and zero receipt sections is bounded legacy fallback; warn, spot-check existing task/proof evidence, and never synthesize/request a receipt. No other status may infer completed review from a notebook.

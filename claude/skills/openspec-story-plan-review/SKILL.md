...
10. Read authoritative `Status:`. If it is `🟣 IN REVIEW`, inspect current Plan, scaffold, blocker, task, and proof evidence, then abort plan review without mutation. Repair evidence takes precedence over fresh review: an implementation/proof deficiency routes singularly to its implementation repair owner; planning/contract drift or malformed state requires one explicit operator reconciliation action because this planning command cannot normalize IN REVIEW; an external blocker routes to its blocker owner. Only when no repair condition survives use the singular route: `Open a completely fresh, oblivious session and run /openspec-story-review <initiative-slug> <story-slug>.`
...
- the implementation `Status:` header in `story.md` is `✅ DONE` — an exact bound receiptless DONE is lifecycle-valid. Receipt presence, absence, shape, verdict, duplication, or staleness cannot affect this route; never synthesize or normalize a receipt. Inspect `Plan:` and current task/proof evidence only to detect contradiction. If Plan is anything other than unambiguous `🟢 PLAN APPROVED`, stop with only `Operator action: investigate and reconcile the contradictory durable Status: ✅ DONE and Plan: <value> state before delivery or archive.` Do not recommend planning commands that reject DONE and do not invent a lifecycle owner. A consistent DONE says "completed stories are not contract-reviewed in place; route new feedback through `/openspec-feedback` as a candidate or explicit reopen decision".
...
**Explicit prohibitions:** never move a story's `Plan:` header into `⚪ TODO`, `🔄 IN PROGRESS`, `🟣 IN REVIEW`, `✅ DONE`, or implementation `⛔ BLOCKED` from this command. Those transitions are not owned by this planning command.
...
For the final authoritative route when Status is IN REVIEW, repeat the repair-first resolution from step 10: surviving implementation/proof deficiency, planning/contract drift or malformed state, and external blocker each route singularly to their evidence-owning repair/reconciliation owner. Only when no repair condition survives give the fresh, oblivious `/openspec-story-review <initiative-slug> <story-slug>` route; the wrapper never launches review. Never collapse IN REVIEW into a blanket fresh-review claim. DONE with non-approved Plan uses only the operator action to investigate/reconcile the contradictory durable state and names no lifecycle owner. If there are no findings, say that explicitly.

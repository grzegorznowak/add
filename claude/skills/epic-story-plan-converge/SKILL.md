...
Coordinate the planning-side ping-pong for exactly one `⚪ TODO` story. This command is an orchestrator only: it starts fresh subagent sessions for `/epic-story-plan-review` and `/epic-story-plan-resume`, preserves their ownership boundaries, keeps only in-memory babysitting notes, and stops when the plan is approved, blocked, no longer eligible, or out of cycle budget.
...
2. If required spec sections are missing or structurally incomplete and there is no newer unaddressed plan-review finding that must be reviewed first, launch a fresh subagent whose task prompt ends with:
...
3. When the story is ready for review, launch a fresh subagent whose task prompt ends with the exact slash command:
...
8. If the decision is `request_changes` or `not_reviewable`, launch a different fresh subagent whose task prompt ends with:

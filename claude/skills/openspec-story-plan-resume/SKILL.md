...
argument-hint: "<initiative-slug> <story-slug> [REPAIR_REF=<planning-path>#<anchor>]"
allowed-tools: Read Edit Write Grep Glob Bash(git status:*) Bash(git log:*) Bash(git worktree:*) Bash(git -C:* remote get-url --all origin) Bash(printf:*) Bash(sha256sum:*)
---
...
Argument: `$ARGUMENTS` — `<initiative_slug> <story_slug> [REPAIR_REF=<planning-path>#<anchor>]`. Pass both positional arguments; this command has no menu fallback. `REPAIR_REF=` is accepted only for the bounded IN REVIEW Mode C route below.
...
- Runtime notebook entry `openspec-review-<repository_key>-<initiative_slug>-<story_slug>` (implementation review findings, optional; derive repository-key-v1 from the resolved OpenSpec root's unique normalized `host/full/owner/path/repository` origin, never checkout basename/path or a per-run key)
...
1. Parse `$ARGUMENTS` into `<initiative-slug>` and `<story-slug>` (both positional, in that order) plus at most one optional `REPAIR_REF=<planning-path>#<anchor>`. Reject unknown or duplicate options. The repair reference must name one current artifact under the resolved change workspace: `story.md`, `proposal.md`, `design.md`, `tasks.md`, or `specs/<relative>.md`; reject absolute paths, `..`, empty fragments, and references outside those planning artifacts. Treat it as a selector, never as evidence by itself.
...
10. Derive the implementation lifecycle status from the `Status:` header field in `<story_file>` and note whether scaffold normalization is needed.
   ...
   - If it is `🟣 IN REVIEW`, preserve that implementation Status and enter **IN REVIEW planning-repair-only** routing. Continue only for a planning-contract/scaffold defect proved by current durable evidence: queued Mode 0 drift, a pending Mode A Plan Review Log finding, a Mode B structural gap, or a `REPAIR_REF=` whose uniquely matching current anchor itself verifies an exact semantic planning-contract defect under Mode C. Operator prose, prior chat, a stale receipt, and the selector alone are insufficient. Do not perform implementation work, alter implementation Status, run plan review, or launch implementation review. If no such defect is durably named and verified, abort with only the completely fresh, oblivious `/openspec-story-review <initiative-slug> <story-slug>` route.
...
12. If the planning lane is `🟢 PLAN APPROVED`, every required spec section is structurally complete, and no scaffold normalization is queued, abort: "this story's plan is already approved; no plan-resume work is needed." **Exception:** when authoritative `Status: 🟣 IN REVIEW` is preserved and a supplied `REPAIR_REF=` uniquely resolves to current planning text that itself verifies an exact semantic contract defect, continue through Mode C even though the Plan is approved and the scaffold is structurally complete. A selector that is missing, ambiguous, stale, merely advisory, or not independently verified cannot bypass this abort.
...
- The story file has no repairable planning work and no scaffold normalization queued while the planning lane is `🟢 PLAN APPROVED` — say "this story is fully planned and approved; no plan-resume work is needed." A verified Mode C `REPAIR_REF=` defect is repairable planning work and must not be classified as this approved/complete early-abort case.

Runtime sections do not block this command. When runtime artifacts exist (`progress.md`), operate in **contract rework mode**: edit only planning spec sections, `## Plan Review Log`, and narrow story scaffold normalization in `story.md`; never edit `progress.md`, source, tests, PR tracking, or implementation `Status:` beyond adding a missing `⚪ TODO` status or normalizing legacy `⬜ TODO` to `⚪ TODO`. In IN REVIEW planning-repair-only routing, repair only the durably evidenced defect, retain `Status: 🟣 IN REVIEW` exactly, and never broaden into implementation or review work.
...
- **Standalone coordinator mode:** runtime-specific guidance may use focused read-only research probes. The coordinator is the one writer for stable planning research and uses whole-page read-modify-write, preserving active and unrelated entries with in-page retirement/compaction.
...
- **Mode 0 — Scaffold normalization**: Required when `Plan:` is missing, `Status:` is missing, `Status:` is exactly `⬜ TODO`, or `## Plan Review Log` is missing. Apply this before Mode A or Mode B:
  ...
  - Re-read `story.md` after normalization before choosing Mode A, Mode B, or Mode C. If scaffold anchors are still missing or ambiguous, stop and report the exact unresolved anchor.
- **Mode A — Feedback absorption**: Required when any entry in `## Plan Review Log` has verdict `request_changes` or `not_reviewable` AND no subsequent "addressed" entry follows it. Entries may come from `/openspec-story-plan-review` or planning feedback routed by `/openspec-feedback`. Process pending entries in chronological order (oldest first). Each addressed finding must produce targeted edits in the authoritative planning artifacts and every affected section identified by that finding, not merely a spec line or mandatory anchor. After Mode A completes, stop. If planning continuation is still needed, the operator re-runs `/openspec-story-plan-resume <initiative-slug> <story-slug>`.
...
- **Mode C — Verified IN REVIEW targeted repair**: Available only when `Status: 🟣 IN REVIEW`, Mode A and Mode B do not apply, and `REPAIR_REF=` resolves to exactly one current anchor in an allowed planning artifact. Re-read the bounded anchor and its directly coupled contract surfaces; continue only when that current evidence demonstrates one exact semantic inconsistency against the source-of-truth hierarchy, required scaffold/artifact shape, or cross-artifact contract. Record the pre-repair anchor text and every directly affected artifact/section. Repair only that defect and the minimum coupled surfaces needed for consistency; do not perform general plan improvement or absorb unrelated feedback. **Mode C overrides every generic Plan-lane downgrade or output rule:** targeted semantic repair preserves `Plan: 🟢 PLAN APPROVED` and `Status: 🟣 IN REVIEW`; it never changes either lane. Append one concise timestamped `Plan Review Log` repair entry naming `REPAIR_REF`, the verified defect, sections edited, and the preserved Plan/Status lanes. Re-read the same anchor and affected surfaces and require exact pre/post evidence that the defect is gone. Missing, ambiguous, unchanged, partial, or broadened evidence is a no-progress stop; do not claim repair. On success, route only to a completely fresh, oblivious `/openspec-story-review <initiative-slug> <story-slug>` session; never launch it or route through plan-converge/plan-review.

A supplied `REPAIR_REF=` outside eligible IN REVIEW Mode C is invalid and must not alter ordinary mode selection. If only Mode 0 applied and the story is otherwise structurally complete, route from the preserved implementation Status. For `Status: 🟣 IN REVIEW`, retain `Status: 🟣 IN REVIEW` and stop singularly with fresh `/openspec-story-review <initiative-slug> <story-slug>` from a new oblivious session; never route through plan-converge or plan-review. For other eligible non-DONE statuses, stop with the planning workflow choice: Converge wrapper `/openspec-story-plan-converge <initiative-slug> <story-slug>` or Non-looped pass `/openspec-story-plan-review <initiative-slug> <story-slug>`. If neither mode applies and the `Plan:` header field is `🟢 PLAN APPROVED`, abort with the "fully planned and approved" message from the readiness check and route by authoritative implementation status. If neither mode applies and the `Plan:` header field is `🟡 PLAN DRAFT` or `🟣 PLAN IN REVIEW`, stop with the same Converge wrapper or Non-looped plan-review choice. If neither mode applies and the `Plan:` header field is `🟠 PLAN CHANGES REQUESTED`, stop with the unresolved plan-review finding that still needs an addressed entry and offer Converge wrapper or the state-correct Non-looped pass `/openspec-story-plan-resume <initiative-slug> <story-slug>`; if no unresolved finding exists, use the Non-looped pass `/openspec-story-plan-review <initiative-slug> <story-slug>`. For every two-path choice, say to choose one and not run both because the wrapper delegates the direct review/resume passes.
...
After cleanup, stop. Outside IN REVIEW, re-run `/openspec-story-plan-resume <initiative-slug> <story-slug>` for Mode B (planning continuation) if needed. In IN REVIEW planning-repair-only routing, retain `Status: 🟣 IN REVIEW` and return the singular operator-run fresh, oblivious `/openspec-story-review <initiative-slug> <story-slug>` route after the named repair; do not launch review or route through plan-converge/plan-review.
...
If validation passes and any spec or proof section changed in Mode A or Mode B, set the `Plan:` header field in `story.md` to `🟡 PLAN DRAFT`. This generic downgrade does not apply to Mode C: a successful targeted semantic repair preserves `Plan: 🟢 PLAN APPROVED` and `Status: 🟣 IN REVIEW` exactly. If scaffold normalization inserted a missing `Plan:` header, the inserted value is `🟡 PLAN DRAFT`. If scaffold normalization only added the missing log anchor or normalized missing/legacy TODO `Status:`, leave any existing `Plan:` value unchanged. Outside Mode C, do not mark the plan approved from this command; `/openspec-story-plan-review` owns transitions into `🟢 PLAN APPROVED`.

## Status and output

**Status transition**: None, except scaffold normalization. The implementation `Status:` header field in `story.md` stays unchanged unless it is missing or exactly legacy `⬜ TODO`; in those two cases this skill writes `Status: ⚪ TODO`. This skill must not rewrite active, in-review, done, blocked, blank, or unknown status values. In Mode A or Mode B it may update the `Plan:` header field in `story.md`: add `Plan: 🟡 PLAN DRAFT` when missing, set it to `🟡 PLAN DRAFT` after spec/proof edits, or leave it unchanged when no plan-affecting edits were needed. Mode C is the explicit exception and preserves the existing approved Plan and IN REVIEW Status.

...
- Inspect the relevant code and tests before the first change. Use the story's `## Actors`, `## Scenarios / Behavior Examples`, `## Verification`, `## Critical Files`, and `## Discovery Notes` to choose the smallest focused seam that covers the next behavior.
- Before choosing the first red seam, build a compact acceptance proof ledger from the story: list every `A<n>` id and, for any acceptance item that names variants, modes, branches, fallback paths, error cases, or examples, list each named case separately. If `## Scenarios / Behavior Examples` contains `S<n> Covers: A<n>`, include the linked scenario case under that acceptance id. Use this ledger to decide tests and implementation order; do not treat one variant or scenario test as covering siblings unless the story explicitly excludes them.

...
- Also build a compact Test Architecture Plan map from `### Test Architecture Plan`: `TAP-*` row id, acceptance slice, layer/scope, owning suite/file, boundary exercised, assertions/observability, fixture/data strategy, CI lane/command, fallback plan, and split/merge rationale. Choose the smallest red seam from this map when it is credible. Do not silently move tests into a different layer/file, change the intended assertion/observable proof signal, collapse unrelated TAP rows into one grab-bag test file, or introduce live/slow/flaky fixture dependencies contrary to the TAP. If source inspection proves the TAP materially wrong, follow the row's fallback plan when it stays within scope; otherwise record the blocker and route to `/epic-story-plan-converge` rather than silently replanning inside claim.

...
- Also build a compact Test Architecture Plan map from `### Test Architecture Plan`: `TAP-*` row id, acceptance slice, layer/scope, owning suite/file, boundary exercised, fixture/data strategy, CI lane/command, and split/merge rationale. Choose the smallest red seam from this map when it is credible. Do not silently move tests into a different layer/file, collapse unrelated TAP rows into one grab-bag test file, or introduce live/slow/flaky fixture dependencies contrary to the TAP. If source inspection proves the TAP materially wrong, record the blocker and route to `/epic-story-plan-converge` rather than silently replanning inside claim.

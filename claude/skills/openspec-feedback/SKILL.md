...
allowed-tools: Read Edit Write Grep Glob Task Bash(gh pr view:*) Bash(gh api:*) Bash(date -u:*) Bash(printf:*) Bash(sha256sum:*) Bash(shasum:*) Bash(git worktree list:*)
---
...
This is a dedicated, single-confirmation intake path. It does not use Phases 0–6 below. The confirmed write sequence is Status-last, resumable, and reconciles canonical postconditions after interruption; it is not all-or-nothing. Review mode starts only from one complete fenced `ADD-REVIEW-PACKET/1` packet supplied as the immediately preceding message or pasted payload. Recognize that exact fence token before treating its contents as ordinary payload; a mention of the token, an excerpt, an unfenced block, or a packet mixed with other feedback stays invalid and must not fall through to ordinary mode. The initiative argument, when supplied, is a binding assertion rather than a second feedback source.
...
3. Semantically cross-check the asserted packet `Verdict` against `Acceptance / proof assessment`, `Verification run`, `Red-first assessment`, `Final stability recheck`, finding count, and finding contents. Reject an inconsistent or contradictory Verdict/proof/stability combination with zero writes and require a fresh packet; do not disposition it. In particular, `APPROVE` requires sufficient passing acceptance/proof and no blocking defect, `REQUEST CHANGES` requires an actionable implementation/proof/contract/quality defect, `BLOCKED` requires an identified external blocker, and `NOT REVIEWABLE` requires a failed entry/prerequisite/integrity/identity/minimum-evidence gate. A claimed stable final recheck cannot erase contrary proof or findings, and reported drift cannot support `APPROVE` or `REQUEST CHANGES`.

A malformed, truncated, semantically contradictory, or internally stale packet fails these grammar/intake gates with zero writes and requires a fresh complete packet. Packet-like input that fails this contract never falls through to ordinary feedback mode.
...
For every proposed canonical change, show the exact artifact and section, the intended postcondition, and why that location is authoritative. Classify whether it changes the story contract, acceptance obligations, proof plan, or proof required to establish implementation readiness. Any such contract, acceptance, or proof edit must include `Plan: 🟠 PLAN CHANGES REQUESTED` in the confirmed postconditions. That planning-authority outcome cannot approve itself, is ineligible for immediate `✅ DONE` or clean completion, and must route to fresh plan review when fully blended/reviewable with no unresolved finding, otherwise plan-resume; implementation resume follows only after Plan approval and a later fresh implementation review. Do not include raw packet text, the fence token, packet verdict metadata, finding IDs as journal keys, source hashes, receipt IDs, review-cycle numbers, or a review-history entry. Durable text records the resulting contract, rationale, owner, or destination—not the packet that prompted it.
...
2. Otherwise, accepted current-story rework is the rework outcome. Any accepted finding owned by the current story publishes `🔄 IN PROGRESS`. Any confirmed contract/acceptance/proof-changing edit also uses this outcome, including when it arises from a reject/waive, defer, or re-scope disposition: downgrade Plan, prohibit immediate DONE, and route planning review/repair before implementation.
3. Only `APPROVE` and `REQUEST CHANGES` packets are eligible for clean completion. When every finding is durably rejected/waived, deferred, or re-scoped, with no accepted current-story work, no accepted external blocker, and no contract/acceptance/proof-changing edit, publish `✅ DONE`.
...
Immediately before showing the complete set for confirmation, freshly re-read and recheck authoritative Plan, `blocked.md`/blocker state, every prerequisite, tasks, proof/readiness evidence, initiative/story binding, and expected target content. Recompute edit classification and lifecycle outcome from those current facts; drift halts with zero writes rather than asking the operator to confirm a stale plan.

Collect and show every finding disposition together, then obtain one explicit confirmation of the complete set before any write. The confirmation view includes operator-added findings (or the explicit zero-finding outcome), every artifact postcondition, every reject/waive rationale, every deferred/re-scoped owner and destination, contract/acceptance/proof classification and Plan transition when applicable, and the one lifecycle outcome. Ask exactly whether to apply the complete triage set.
...
4. Immediately before publishing clean completion as `✅ DONE`, freshly re-read and recheck, in this order: authoritative Plan, `blocked.md`/blocker state, every prerequisite, tasks, and proof/readiness evidence. DONE requires an unambiguous approved Plan, no blocker, satisfied prerequisites, complete valid tasks, sufficient current proof/readiness, no accepted rework, and no contract/acceptance/proof-changing edit. Any drift or contradiction stops before Status publication with zero further writes, keeps Status `🟣 IN REVIEW`, and routes its owning repair or review workflow.
5. Immediately before DONE Status publication, as the final pre-Status gate, perform a fresh packet replay rather than treating the submitted packet or confirmation as cached authority. Freshly re-evaluate current implementation evidence with the same `/openspec-story-review` readonly evaluator semantics. Launch exactly one isolated, fresh **Task child** to perform that replay; do not role-play the evaluator in this feedback writer's context. Give the child only the readonly evaluator invocation arguments (the canonical initiative/story slugs and any required `WORKTREE=` artifact selectors) plus the instruction to apply the canonical `/openspec-story-review` semantics to current OpenSpec artifacts and readable live repository evidence. The child has only the readonly evaluator's `Read`, `Grep`, and `Glob` evidence boundary and returns its complete transient `ADD-REVIEW-PACKET/1` result to this parent invocation.

   The Task child must not receive or use the submitted packet, dispositions, confirmation, prior chat, implementation-session or parent context, implementation summaries, notebook references, or operational notes. Freshly re-evaluate the current implementation and its bound proof, artifacts, and lifecycle state using the same immutable, fresh-oblivious, read-only evidence boundary and the same `/openspec-story-review` readonly evaluator semantics for verdicts, findings, and final stability. Derive the evaluator's complete fresh packet in child memory, including all scalar assessments and finding substance; the parent may use the returned packet only for the equivalence decision below.

   Require the fresh evaluation to be semantically equivalent to the submitted packet before DONE remains eligible. Compare review mode/focus and coverage; bound root, initiative, story, and subject; Verdict; acceptance/proof, verification, red-first, and final-stability assessments; and the complete finding set by severity, summary, evidence, impact, proof/verification, and requested outcome. Presentation-only differences may be ignored solely when they preserve meaning: lexical spelling of the same resolved root, finding order, local finding IDs, and wording or whitespace that changes none of the compared facts, scope, evidence, certainty, severity, or requested outcomes. Do not ignore an omitted, added, weakened, strengthened, newly uncertain, or otherwise substantively changed assessment or finding.

   A fresh replay that is not semantically equivalent to the submitted packet is a mismatch: reject it, stop with zero writes permitted after rejection, and do not publish DONE. The same rejection applies when the fresh result reports drift. This mismatch exit permits zero writes after rejection—make zero further writes, keep top-level Status `🟣 IN REVIEW`, report any non-Status postconditions already applied by this pass, and route to a fresh review/packet; never roll those partial edits back or pretend they did not occur. The fresh evaluation and fresh packet remain transient in memory: never persist, write, or create a packet receipt, packet digest, packet identity, review history, packet history, or the packet itself.
6. Write the top-level `Status:` header last and immediately after the clean-completion replay succeeds, only after every other confirmed write and recheck is verified. Change only that exact header: blocker outcome to `⛔ BLOCKED`, rework outcome to `🔄 IN PROGRESS`, and eligible clean completion to `✅ DONE`. Re-read it and all final postconditions before reporting success.
...
- Update `story.md` Status header to `🔄 IN PROGRESS` for implementation-review feedback that should drive immediate story resume or PR rework. Any prior `progress.md → ## Implementation Review Receipt` is inert legacy evidence; do not delete, rewrite, normalize, or use it to override current Status. The explicit migration command owns removal of a uniquely recognized legacy receipt. Persist ordinary-feedback identity and routing in the `<receipt_root>` initiative ledger after owned edits succeed.
...
Record the before/after status in the feedback checkpoint and paired ordinary-feedback initiative receipt. The FB-tagged reopen checkpoint is durable evidence of the current routing. Any legacy `## Implementation Review Receipt` remains inert and unchanged; it cannot contradict current Status, and this command does not repair or remove it.
...
- If authoritative Status is IN REVIEW, inspect the current Plan, scaffold integrity, `blocked.md`/blocker state, tasks, and implementation proof before selecting the final route. Repair deficiencies or drift take precedence: planning/contract or scaffold repair requires one explicit operator reconciliation action because planning commands reject IN REVIEW; an external blocker routes to its blocker owner; incomplete tasks or insufficient proof route singularly to the implementation repair owner. Only when no repair condition survives give the fresh oblivious `/openspec-story-review <initiative> <story-slug>` handoff; the wrapper never launches review.

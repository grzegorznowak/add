...
6. **Build `<project_root_map>` from recorded + explicit entries**. Initialize empty. For each `<basename>` in the union of `<recorded_worktree_map>` keys and `<explicit_worktree_map>` keys:
   ...
   - Resolve `<target_repo>` for `<basename>`: `<workspace_root>/projects/<basename>` if `<basename>` resolves to a sub-repo, else `<workspace_root>` if it matches `basename(<workspace_root>)`. If neither matches but the effective path resolves to `<workspace_root>` and `<workspace_root>` is itself a git repo, treat `<basename>` as a root-repo alias for the original checkout and set `<target_repo>` = `<workspace_root>`. If none of those rules resolves, abort with "claimed worktree for `<basename>` cannot be matched to any repo on disk".
...
8. **Done**. `<project_root_map>` is set. All downstream resolution uses these rules:
   ...
   - Code in the root repo outside `openspec/...` and outside `projects/<name>/...` → if `<project_root_map>` has `basename(<workspace_root>)`, route the same relative path to that mapped root; else if a consumed root-repo alias key exists in `<project_root_map>` and its value resolves to `<workspace_root>`, route through that mapped root; else use `<workspace_root>/<relative-path>`.
   - Git commands targeting repo `<name>`: `git -C <project_root_map>[<name>] ...`. For root-repo git commands, use `basename(<workspace_root>)` when present, otherwise use the consumed root-repo alias key whose mapped value resolves to `<workspace_root>`, otherwise use `<workspace_root>`.

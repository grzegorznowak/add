...
allowed-tools: Read Edit Write Grep Glob Task Bash Bash(git -C:* remote get-url --all origin) Bash(printf:*) Bash(sha256sum:*)
---
...
If this command is being considered from the same session that just wrote or converged the implementation, stop and open a completely fresh session first. The review itself must never consume parent/converger notebook, chat, summary, handoff, or orientation context under any alias. Supplemental review-orientation spawning is optional and fail-closed: use it only when the runtime can establish an isolated child boundary before launch and provide parent-verifiable boundary proof independent of the child's own claims. Current Pi spawn exposes notebook tools plus automatic notebook page-name/preview material, so it does not establish that boundary; skip supplemental review-orientation spawn there and perform the review from current canonical artifacts plus live inspection. A child echo or self-attested no-tool statement is not isolation proof. If a future/runtime-isolated child boundary is available, generate a cryptographically unpredictable fresh per-invocation review run token in memory and a review-run sentinel derived from it; never persist either. Put both in every isolated review-research prompt, prohibit notebook tools and nested spawn, and require exact echo plus the runtime's independent boundary proof. Discard missing/stale/mismatched or boundary-unproven evidence. Firewall parent/converger notebook references/pages/lists/previews, research, ops or operational context, handoff, compact excerpts, renamed selectors, implementation summaries/context, convergence summaries/context, relabeled implementation or convergence context, evidence bundles, orientation, digests, briefings, and prior chat. The menu still makes operator-explicit selection possible, but the friction is intentional and any future change that adds silent auto-inference or implementation-context handoff here must be rejected.
...
1. Parse `$ARGUMENTS`. Validate each supplied or menu-selected initiative/story slug against `^[a-z0-9]+(?:-[a-z0-9]+)*$` before interpolating it into a path; reject malformed values. Record `<explicit_pair>` as true only when both positional slugs were supplied in this invocation before any menu fallback.
   ...
   Set `<workspace_root>` = `<cwd>` and `<openspec_root>` = `<workspace_root>`. `<workspace_root>` is the launch checkout/root-repo discovery base and is never re-anchored. `<openspec_root>` is the transient artifact anchor for `openspec/...` and may reroot in memory. There is no persisted `OpenSpec root:` field. Derive `<repository_key>` only after the root preflight below.
...
9. Optional prior notebook entry `openspec-review-<repository_key>-<initiative_slug>-<story_slug>` if it exists — supplemental sourced orientation only after durable artifacts; derive the stable repository qualifier from canonical origin, never a checkout path/basename, and it cannot override or replace the receipt
...
5. For every completed verdict, update the top-level `Status:` header and write the compact durable `progress.md → ## Implementation Review Receipt`; optional supplemental findings use stable page `openspec-review-<repository_key>-<initiative_slug>-<story_slug>` under the ordered persistence protocol.
...
- Name the exact missing evidence/contract issue and route to the owner that can repair it; do not default back to this review command merely because authoritative Status remains `🟣 IN REVIEW`:
  ...
  - missing anchors or incomplete/non-reviewable planning scaffold -> singular direct `/openspec-story-plan-resume <initiative-slug> <story-slug>` for structural Mode 0/B planning-contract/scaffold-only repair that preserves authoritative `Status: 🟣 IN REVIEW` and returns to operator-run fresh oblivious review without launching it, or `/openspec-story-plan INITIATIVE=<initiative-slug>` when the change workspace is absent. If the scaffold is structurally complete and current planning evidence instead proves one exact semantic contract defect, resolve its unique live artifact path and anchor and route with the exact verified `REPAIR_REF=<planning-path>#<anchor>` by putting `/openspec-story-plan-resume <initiative-slug> <story-slug> REPAIR_REF=<verified-planning-path>#<verified-anchor>` on the scalar **Suggested next action:** line so direct plan-resume enters Mode C. Replace both verified placeholders with the concrete values; never emit the placeholder form or rely on chat context. Structural Mode 0/B routes omit `REPAIR_REF` only when their entry conditions actually apply;
...
- Use focused child agents only when the runtime first establishes an isolated boundary that excludes parent/converger notebook, chat, preview, summary, and handoff context and provides independent boundary proof. Each child is read-only for code and coordination files; runtime-specific invocation details belong in runtime-specific fragments. Child self-attestation is not boundary proof.
- A documented manual focused-pass substitute is required when isolated child spawning fails, times out, is unavailable, or exposes notebook tools/automatic previews. The substitute reviews current canonical artifacts and live code only and records the pass title, substitution reason, files/symbols inspected, searches/direct reads used, findings, and explicit clean or inconclusive result.
...
2. For `BLOCKED`, create or update `<change_dir>/blocked.md` first with the blocker, evidence, resolution condition, and next owner. If that fails, leave receipts and Status unchanged and report the failure. Complete any optional review notebook persistence now, before receipt and timeline publication; it is supplemental.
...
When notebook tools are available, the operator MAY persist review findings only to stable record `openspec-review-<repository_key>-<initiative_slug>-<story_slug>`, using repository-key-v1 derived after root resolution as above. Missing/differing origin identities or key drift skips/fails closed for this optional persistence only; the artifact-based review continues. Never use a worktree basename/path or per-run record/key. Review is the single coordinator writer: before writing, read the whole existing page, perform read-modify-write, preserve all active entries and unrelated content, and write the complete merged page. Use in-page retirement and in-page compaction rather than deletion/reset/topic assumptions. Optional persistence must occur before receipt and timeline publication and final Status; no write after Status. It is supplemental and not authoritative for downstream skills. The top-level `Status:` header and `progress.md → ## Implementation Review Receipt` are the durable completed-verdict signals; canonical artifacts outrank notebook orientation.
